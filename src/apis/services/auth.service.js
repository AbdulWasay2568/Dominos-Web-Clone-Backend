...
const registerUser = (input) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const existingUser = yield client_1.prisma.users.findUnique({ where: { email } });
    if (existingUser) {
    ...
    const token = jsonwebtoken_1.default.sign({ id: newUser.id, role: newUser.role }, process.env.JWT_SECRET, { expiresIn: "1h" });
...
const loginUser = (input) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const user = yield client_1.prisma.users.findUnique({ where: { email } });
